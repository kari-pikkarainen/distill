...
"$repo/test/audit/render-audit-page.sh" "$evidence_dir"

# ── 8. Benchmark ───────────────────────────────────────────────────────────
step "run multi-profile benchmark (base + java21 + chainguard)"
"$repo/test/bench/run-profiles.sh" --out "$repo/test/bench/report"
...
# Landing index — links to audit, benchmark, and use-case decision guide.
cat > "$site_root/index.html" <<HTML
<!doctype html>
<html lang="en"><meta charset="utf-8">
<title>distill — local (Wave 0)</title>
<style>
  body{font:15px/1.55 system-ui,sans-serif;margin:2rem auto;max-width:900px;color:#222}
  h1{font-size:1.5rem;margin-bottom:.2rem}
  h2{font-size:1.1rem;margin-top:2rem;border-bottom:1px solid #ddd;padding-bottom:.2rem}
  .meta{color:#666;font-size:.88rem}
  ul{padding-left:1.2rem}
  li{margin:.4rem 0}
  code{background:#eef0f3;padding:.1rem .3rem;border-radius:3px;font-size:.88em}
  pre{background:#1e293b;color:#e2e8f0;padding:.8rem 1rem;border-radius:6px;font-size:.88em;line-height:1.5;overflow-x:auto}
</style>
...
<h2>Pages</h2>
<ul>
  <li><a href="images/base-ubi9/"><strong>Audit evidence · base-ubi9</strong></a> — SBOM, CVE scan, compliance framework mapping, offline VERIFY.sh</li>
  <li><a href="benchmarks/"><strong>Benchmark · all profiles</strong></a> — distill vs. UBI9 variants vs. Chainguard, measured numbers per use case</li>
</ul>

<h2>Pull the images</h2>
<pre>docker pull localhost:5555/base-ubi9:latest
docker pull localhost:5555/java21-ubi9:latest

cosign verify --key local/keys/cosign.pub \\
  --allow-insecure-registry --allow-http-registry \\
  localhost:5555/base-ubi9:latest</pre>

<h2>Which image should I pick?</h2>
<p>See <a href="../../docs/use-case-matrix.md"><code>docs/use-case-matrix.md</code></a>
in the repo for the decision guide backed by the measurements on the benchmark page.</p>

<h2>Rebuild the stack</h2>
<pre>devbox run mvp-local          # full pipeline
devbox run bench-profiles     # just the benchmarks
devbox run quality-gate ...   # just the gate checks</pre>
HTML
