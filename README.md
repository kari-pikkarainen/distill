...
If you want to see the whole supply-chain story end-to-end on your laptop —
build → push → sign → SBOM → CVE scan → audit evidence bundle → benchmark —
without standing up any cloud infrastructure, the local stack does it in
one command:
...
That spins up a local OCI registry on `localhost:5555`, generates a Cosign
keypair, builds and pushes [`specs/base-ubi9.distill.yaml`](./specs/base-ubi9.distill.yaml),
runs the quality gate (size budget, SBOM completeness, signature
verification), generates an audit evidence bundle with offline-verifiable
manifest signatures, and runs a measured benchmark against stock UBI9 and
Chainguard images.

Once it finishes:

```bash
devbox run serve            # serves the landing page on http://localhost:8088
devbox run mvp-local-down   # tears it all down
```

Three pages worth visiting:

- **`/`** — landing page with links and pull commands
- **`/images/base-ubi9/`** — the audit bundle: SBOM, CVE scan, compliance
  framework mapping, signed manifest, offline `VERIFY.sh`
- **`/benchmarks/`** — measured comparison against UBI9 stock, ubi9-minimal,
  ubi9-micro, Chainguard wolfi-base, plus a Java 21 JRE profile

The full design rationale is in [`docs/mvp-plan.md`](./docs/mvp-plan.md);
the test bench specification is in [`docs/testing-strategy.md`](./docs/testing-strategy.md).
...
## Examples and product specs

Two directories hold image specs, with different intent:

- [`examples/`](./examples/) — single-distro reference specs that pair with
  `container-structure-test` configurations. Use these when you want to
  understand the spec format or test the build pipeline end-to-end.
  - [`rhel9-runtime/`](./examples/rhel9-runtime/) — minimal RHEL9/UBI9, target ≤30 MB
  - [`debian-runtime/`](./examples/debian-runtime/) — minimal Debian Bookworm, target ≤20 MB
- [`specs/`](./specs/) — canonical product image specs targeting the local
  registry (`localhost:5555`). Used by `devbox run mvp-local` and intended to
  evolve into the published catalog. Each spec has a sidecar `*.quality.yaml`
  declaring the size budget and CVE thresholds, and an optional
  `*.exceptions.yaml` documenting accepted CVEs with rationale.
  - `base-ubi9.distill.yaml` — minimal UBI9 base with shell + coreutils
  - `base-ubi9-minimal.distill.yaml` — even leaner variant for static binaries
  - `java21-ubi9.distill.yaml` — Java 21 JRE on UBI9 (OpenJDK)
...
| Document | Topic |
|---|---|
| [`docs/strategy.md`](./docs/strategy.md) | Commercial strategy — product tiers, pricing, go-to-market, MVP scope, risks |
| [`docs/testing-strategy.md`](./docs/testing-strategy.md) | Three-layer test bench (quality gate, benchmark harness, security program) and the 8 non-negotiable per-build checks |
| [`docs/mvp-plan.md`](./docs/mvp-plan.md) | Phased build order — 4 waves over 28 weeks with explicit kill-criteria per wave |
| [`docs/use-case-matrix.md`](./docs/use-case-matrix.md) | "Which image should I pick?" decision guide backed by measured benchmark data |
| [`docs/image-hardening.md`](./docs/image-hardening.md) | Immutability, access control, runtime security context, CVE-reduction strategy with measured floor |

Companion READMEs live next to the code they describe:
[`local/README.md`](./local/README.md) (local stack),
[`test/README.md`](./test/README.md) (test bench),
[`examples/README.md`](./examples/README.md) (example specs).
